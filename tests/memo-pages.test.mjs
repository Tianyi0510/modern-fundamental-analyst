...
test("memo article uses the source document prose and the wider references layout", async () => {
  ...
  assert.match(
    styles,
    /\.memo-references\s*\{[^}]*margin-top:\s*var\(--memo-conclusion-space-after\);[^}]*padding:\s*var\(--space-section-compact\) 48px;[^}]*background:\s*var\(--background-gray\)/s,
  );
...
test("desktop and touch memo interactions share restrained color and scale feedback", async () => {
  ...
  assert.match(
    css,
    /\.metric,\s*\.memo-card,\s*\.performance-summary > div\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-primary\)/s,
  );
  ...
  assert.match(
    css,
    /\.memo-card:hover,\s*\.memo-card:focus-visible\s*\{[^}]*color-mix\(in srgb, var\(--bright-blue\) 22%, var\(--white\)\);[^}]*scale\(var\(--motion-scale-press\)\)/s,
  );
  assert.doesNotMatch(css, /\.memo-card:hover,\s*\.memo-card:focus-visible\s*\{[^}]*translateY/);
  assert.match(
    css,
    /\.memo-index-row:hover,\s*\.memo-index-row:focus-visible\s*\{[^}]*color-mix\(in srgb, var\(--bright-blue\) 18%, var\(--white\)\);[^}]*scale\(var\(--motion-scale-press\)\)/s,
  );
  assert.match(css, /\.memo-index-row\s*\{[^}]*align-items:\s*start/s);
  assert.doesNotMatch(index, /memo-index-row[\s\S]*?className="arrow-icon"/);
  assert.match(css, /--motion-scale-press:\s*0?\.98;/);
  for (const selector of [
    String.raw`\.memo-card`,
    String.raw`\.memo-index-row`,
    String.raw`\.memo-disclosure > summary`,
  ]) {
    assert.match(
      css,
      new RegExp(selector + String.raw`:active\s*\{[^}]*transform:\s*scale\(var\(--motion-scale-press\)\)`, "s"),
    );
  }
  assert.match(
    css,
    /@media \(hover: none\) and \(pointer: coarse\)[\s\S]*?\.memo-card:not\(\.memo-card-placeholder\):active\s*\{[^}]*transform:\s*scale\(var\(--motion-scale-press\)\)/s,
  );
  assert.match(
    css,
    /@media \(hover: none\) and \(pointer: coarse\)[\s\S]*?\.memo-index-row:active\s*\{[^}]*transform:\s*scale\(var\(--motion-scale-press\)\)/s,
  );
  assert.doesNotMatch(css, /\.memo-disclosure > summary::before/);
  assert.match(
    css,
    /\.memo-disclosure > summary:hover,\s*\.memo-disclosure > summary:focus-visible\s*\{[^}]*background:\s*color-mix\(in srgb, var\(--bright-blue\) 18%, var\(--white\)\);[^}]*color:\s*var\(--deep-blue\)/s,
  );
  assert.match(
    css,
    /\.memo-disclosure > summary:hover > span:first-child,[^}]*translateX\(var\(--motion-offset-summary\)\)/s,
  );
  ...
  assert.match(
    css,
    /\.memo-disclosure\[open\]:not\(\[data-closing\]\) > summary svg\s*\{\s*transform:\s*rotate\(180deg\);/,
  );
  assert.match(
    css,
    /\.memo-disclosure > summary:hover,[\s\S]*?\.memo-disclosure > summary:focus-visible\s*\{\s*color:\s*var\(--deep-blue\)/s,
  );
  assert.match(
    css,
    /@media \(hover: none\) and \(pointer: coarse\)[\s\S]*?\.memo-disclosure > summary:active\s*\{[^}]*background:\s*color-mix\(in srgb, var\(--bright-blue\) 18%, var\(--white\)\);[^}]*color:\s*var\(--deep-blue\)/s,
  );
  assert.match(
    css,
    /@media \(hover: none\) and \(pointer: coarse\)[\s\S]*?\.memo-disclosure > summary:active > span:first-child\s*\{[^}]*transform:\s*none/s,
  );
});
