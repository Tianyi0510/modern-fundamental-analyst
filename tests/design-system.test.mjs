...
test("editorial copy preserves authored casing and mobile arrows use intentional touch motion", async () => {
  ...
  assert.match(responsive, /\.home-page \.round-link:active\s*\{[^}]*transform:\s*scale\(\.96\);/);
  assert.match(responsive, /\.home-page \.round-link:active \.arrow-icon\s*\{\s*transform:\s*scale\(1\.08\);/);
...
test("mobile navigation uses coordinated motion with a reduced-motion fallback", async () => {
  ...
  assert.match(css, /\.mobile-menu-button:hover, \.mobile-menu-button:focus-visible\s*\{[^}]*background:\s*var\(--bright-blue\);[^}]*transform:\s*scale\(var\(--motion-scale-hover\)\)/s);
  assert.match(css, /\.mobile-menu-close:hover, \.mobile-menu-close:focus-visible\s*\{[^}]*background:\s*var\(--bright-blue\);[^}]*transform:\s*scale\(var\(--motion-scale-hover\)\)/s);
  ...
  assert.match(css, /@media \(max-width:\s*800px\)[\s\S]*?\.site-footer\s*\{[^}]*gap:\s*0;[^}]*padding:\s*var\(--space-8\) 0/s);
  assert.match(css, /@media \(max-width:\s*800px\)[\s\S]*?\.footer-link-groups\s*\{[^}]*grid-template-columns:\s*minmax\(0, \.8fr\) minmax\(0, 1\.2fr\)/s);
  assert.match(css, /@media \(max-width:\s*800px\)[\s\S]*?\.footer-bottom\s*\{[^}]*margin-top:\s*0/s);
