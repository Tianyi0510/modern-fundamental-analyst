...
test("mobile navigation uses coordinated motion with a reduced-motion fallback", async () => {
  ...
  assert.match(css, /@keyframes mobile-menu-control-enter\s*\{[\s\S]*?rotate\(-4deg\) scale\(\.94\)[\s\S]*?rotate\(1deg\) scale\(1\.02\)[\s\S]*?transform:\s*none/s);
  ...
  assert.match(css, /@media \(max-width:\s*800px\)[\s\S]*?\.footer-bottom\s*\{[^}]*padding:\s*var\(--space-8\) 0/s);
