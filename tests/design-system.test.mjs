...
test("typography uses centralized semantic role tokens", async () => {
  ...
  assert.match(base, /--font-size-compact-title:\s*clamp\(1\.25rem, 1\.125rem \+ 0?\.5vw, 1\.5rem\)/);
  ...
  assert.match(base, /--font-size-caption:\s*0?\.9375rem/);
  assert.match(base, /--font-size-data-ring:\s*2\.5rem/);
  assert.match(base, /--tracking-heading:\s*-0?\.05em/);
  assert.match(base, /--tracking-body:\s*0/);
  assert.match(base, /--tracking-label:\s*0?\.05em/);
...
test("editorial color roles keep the footer inverse and Medium Blue auxiliary", async () => {
  ...
  assert.match(css, /--text-secondary:\s*rgba\(0,\s*0,\s*0,\s*0?\.62\)/);
  assert.match(css, /--text-tertiary:\s*rgba\(0,\s*0,\s*0,\s*0?\.56\)/);
  assert.match(css, /--text-inverse-secondary:\s*rgba\(255,\s*255,\s*255,\s*0?\.68\)/);
  assert.match(css, /--interactive-accent:\s*var\(--medium-blue\)/);
  assert.match(
    css,
    /\.site-footer\s*\{[^}]*background:\s*var\(--surface-inverse\)[^}]*color:\s*var\(--text-inverse\)/s,
  );
  ...
  assert.match(
    css,
    /\.home-page \.cta\s*\{[^}]*background:\s*var\(--surface-highlight\)[^}]*color:\s*var\(--text-primary\)/s,
  );
  assert.match(
    css,
    /\.home-page \.cta \.button-dark\s*\{[^}]*background:\s*var\(--black\)[^}]*color:\s*var\(--white\)/s,
  );
  assert.match(
    css,
    /\.portfolio-page \.portfolio-kpis > div\[data-tone="plain"\]\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-primary\)/s,
  );
  assert.match(
    css,
    /\.portfolio-page \.portfolio-kpis > div\[data-tone="highlight"\]\s*\{[^}]*background:\s*var\(--surface-highlight\);[^}]*color:\s*var\(--text-brand\)/s,
  );
  assert.match(
    css,
    /\.portfolio-page \.portfolio-kpis > div\[data-tone="brand"\]\s*\{[^}]*background:\s*var\(--surface-brand\);[^}]*color:\s*var\(--text-highlight\)/s,
  );
  assert.match(
    css,
    /\.portfolio-page \.portfolio-kpis > div\[data-tone="paper"\]\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-brand\)/s,
  );
  assert.match(
    css,
    /\.home-page \.metric-band > \.metric\[data-tone="highlight"\],\s*\.performance-page \.performance-summary > div\[data-tone="highlight"\]\s*\{[^}]*background:\s*var\(--surface-highlight\);[^}]*color:\s*var\(--text-brand\);/s,
  );
  assert.match(
    css,
    /\.about-boundaries > article:first-child[^}]*\{[^}]*background:\s*var\(--surface-inverse\);[^}]*color:\s*var\(--text-inverse\);/s,
  );
  assert.match(
    css,
    /\.about-boundaries > article:last-child\s*\{[^}]*background:\s*var\(--surface-highlight\);[^}]*color:\s*var\(--text-primary\);/s,
  );
  assert.match(
    css,
    /\.home-page \.metric-band > \.metric\[data-tone="brand"\],\s*\.performance-page \.performance-summary > div\[data-tone="brand"\]\s*\{[^}]*background:\s*var\(--surface-brand\);[^}]*color:\s*var\(--text-highlight\);/s,
  );
  assert.match(
    css,
    /\.home-page \.metric-band > \.metric\[data-tone="paper"\],\s*\.performance-page \.performance-summary > div\[data-tone="paper"\]\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-brand\);/s,
  );
  ...
  assert.match(
    css,
    /\.performance-home\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-primary\)/s,
  );
  assert.match(
    css,
    /\.performance-home \.button-white\s*\{[^}]*background:\s*var\(--black\);[^}]*color:\s*var\(--white\)/s,
  );
  assert.match(
    css,
    /\.contact-grid > article:first-child[^}]*\{[^}]*background:\s*var\(--surface-inverse\);[^}]*color:\s*var\(--text-inverse\)/s,
  );
  assert.match(css, /\.contact-grid > article:first-child p\s*\{[^}]*color:\s*var\(--text-inverse\)/s);
  assert.match(
    css,
    /\.contact-grid > article:nth-child\(2\)\s*\{[^}]*background:\s*var\(--surface-primary\);[^}]*color:\s*var\(--text-primary\)/s,
  );
  ...
  assert.doesNotMatch(
    css,
    /\.(?:site-header|home-opening|home-about|page-hero|about-section|about-boundaries|about-closing|portfolio-kpis|portfolio-holdings-heading|methodology|contact-grid|site-footer)\s*\{[^}]*(?:border-top|border-bottom):/s,
  );
  ...
  assert.doesNotMatch(
    css,
    /\.performance-summary > div\[data-tone="paper"\][^{]*\{[^}]*box-shadow:\s*inset 0 0 0 1px var\(--black\)/s,
  );
});
...
test("editorial copy preserves authored casing and mobile arrows use intentional touch motion", async () => {
  ...
  assert.match(
    typography,
    /\.hero-bottom > p,[\s\S]*?\.legal \.legal-subtitle\s*\{[\s\S]*?font-size:\s*var\(--font-size-lead\);[\s\S]*?text-transform:\s*none;/,
  );
  assert.doesNotMatch(`${typography}\n${responsive}\n${subscribe}\n${contact}`, /text-transform:\s*capitalize/);
  assert.match(
    responsive,
    /\.arrow-icon\s*\{[^}]*font-weight:\s*var\(--weight-bold\);[^}]*-webkit-text-stroke:\s*0?\.45px currentColor;[^}]*transition:\s*transform/s,
  );
  assert.match(
    responsive,
    /\.home-page \.text-link:active \.arrow-icon,[\s\S]*?\.allocation-card > a:active \.arrow-icon\s*\{\s*transform:\s*translateX\(5px\);/,
  );
  assert.match(
    responsive,
    /\.home-page \.round-link:active\s*\{[^}]*transform:\s*scale\(var\(--motion-scale-press\)\);/,
  );
  ...
  assert.match(
    chrome,
    /\.home-page \.text-link \.arrow-icon,\s*\.home-page \.allocation-card > a \.arrow-icon\s*\{[^}]*stroke-width:\s*3;/,
  );
  assert.match(chrome, /\.site-header \.wordmark,[\s\S]*?\.site-footer \.wordmark\s*\{\s*transition:\s*none;/);
  assert.match(
    chrome,
    /\.site-header \.wordmark:hover,[\s\S]*?\.site-footer \.wordmark:active\s*\{[^}]*color:\s*inherit;[^}]*transform:\s*none;/s,
  );
  ...
  assert.match(
    subscribe,
    /\.submit:hover:not\(:disabled\),\s*\.submit:focus-visible:not\(:disabled\)\s*\{[^}]*background:\s*var\(--bright-blue\);[^}]*color:\s*var\(--black\)/,
  );
});
...
test("page sections share one responsive vertical rhythm", async () => {
  ...
  assert.match(
    css,
    /@media \(max-width: 800px\)[\s\S]*--space-page-gutter:\s*var\(--space-4\);[\s\S]*--space-section:\s*var\(--space-10\);[\s\S]*--space-section-compact:\s*var\(--space-9\);[\s\S]*--space-heading-content:\s*var\(--space-7\);[\s\S]*--space-related-content:\s*var\(--space-6\);/,
  );
  ...
  assert.match(css, /--motion-ease-standard:\s*cubic-bezier\(0?\.2, 0, 0, 1\)/);
  assert.match(css, /--motion-ease-emphasized:\s*cubic-bezier\(0?\.22, 1, 0?\.36, 1\)/);
  ...
  assert.match(css, /\.legal-section\s*\{[^}]*padding:\s*0/s);
  assert.match(
    css,
    /\.memo-section \+ \.memo-section\s*\{[^}]*margin-top:\s*var\(--space-section\);[^}]*padding-top:\s*var\(--space-section-compact\)/s,
  );
  ...
  assert.match(
    css,
    /@media \(max-width:\s*800px\)[\s\S]*?\.performance-page \.performance-summary > div\s*\{[^}]*min-height:\s*168px;[^}]*padding:\s*var\(--space-5\) var\(--space-page-gutter\)/s,
  );
  assert.match(
    css,
    /@media \(max-width:\s*800px\)[\s\S]*?\.performance-page \.methodology\s*\{[^}]*gap:\s*var\(--space-heading-content\)/s,
  );
  assert.doesNotMatch(`${css}\n${contact}`, /100vmax|clip-path:\s*inset\(0 -100vmax\)/);
  assert.match(
    base,
    /\.hero::before,[\s\S]*?\.site-footer::before\s*\{[^}]*inset:\s*0 calc\(50% - 50vw\);[^}]*background:\s*inherit;/s,
  );
