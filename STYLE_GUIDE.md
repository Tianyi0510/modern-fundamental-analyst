...
The mobile close icon replays its entrance keyframes on each opening, including after navigation. Memo and monthly-data disclosures share `AnimatedDisclosure` and keep native details/summary semantics and use a small client wrapper to animate height in both directions, including WebKit. Repeated input reverses from the current height; reduced motion switches immediately. Without JavaScript, the native disclosure remains usable.
