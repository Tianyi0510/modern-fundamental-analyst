...
export default async function MemoPage({ params }: Props) {
  ...
  const memo = memos.find((item) => item.slug === slug);
  if (!memo) notFound();
  return <main className="memo-detail-page"><SiteHeader counterpartPath={`/zh-tw/memos/${slug}`} />
    <article className="memo-article shell"><Link className="back-link" href="/memos">← All Investment Memos</Link><p className="eyebrow"><span /> {memo.tag}</p><h1>{memo.title}</h1><div className="article-meta"><span>{memo.date}</span><span>{memo.readTime}</span><span>Investment Memo {memo.number}</span></div>
