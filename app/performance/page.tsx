...
export default function PerformancePage() {
  return <main><SiteHeader counterpartPath="/zh-tw/performance" />
    ...
    <section className="returns shell"><div className="section-heading"><p className="section-number">Annual returns</p><h2>Year by year.</h2></div>
      <div className="returns-table"><div className="table-head"><span>Year</span><span>Portfolio</span><span>Benchmark</span></div>{annualReturns.map((r) => <div className="return-row" key={r.year}><span>{r.year}</span><strong className={r.portfolio < 0 ? "negative" : ""}>{r.portfolio > 0 ? "+" : ""}{r.portfolio}%</strong><span>{r.benchmark > 0 ? "+" : ""}{r.benchmark}%</span></div>)}</div>
    </section>
