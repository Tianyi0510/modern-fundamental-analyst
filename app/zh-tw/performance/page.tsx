...
export default function PerformancePageZhTw() {
  return <main><SiteHeader locale="zh-tw" counterpartPath="/performance" />
    ...
    <section className="performance-summary shell"><div className="summary-primary"><span>累積報酬</span><strong className="positive">+18.6%</strong><small>成立以來</small></div><div><span>基準</span><strong className="positive">+12.4%</strong><small>相同期間</small></div><div><span>最大回撤</span><strong className="negative">−11.2%</strong><small>高點至低點</small></div></section>
    <section className="returns shell"><div className="section-heading"><p className="section-number">年度報酬</p><h2>逐年檢視。</h2></div>
      <div className="returns-table"><div className="table-head"><span>年度</span><span>投資組合</span><span>基準</span></div>{annualReturnsZhTw.map((row) => <div className="return-row" key={row.year}><span>{row.year}</span><strong className={row.portfolio < 0 ? "negative" : "positive"}>{row.portfolio > 0 ? "+" : ""}{row.portfolio}%</strong><span className={row.benchmark < 0 ? "negative" : "positive"}>{row.benchmark > 0 ? "+" : ""}{row.benchmark}%</span></div>)}</div>
    </section>
