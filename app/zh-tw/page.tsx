...
const portfolioDate = formatDate(portfolioSnapshot.asOf, "zh-tw");
  ...
  return (
    <main className="home-page">
      ...
      <section className="metric-band shell" aria-label="投資組合摘要">
        <div className="metric metric-featured"><span>累積報酬</span><strong>{formatPercent(portfolioSnapshot.totalReturn)}</strong><small>成本基礎累積報酬</small></div>
        ...
        <div className="metric metric-accent"><span>投資組合 XIRR</span><strong>{formatPercent(portfolioSnapshot.xirr)}</strong><small className="date-text">截至 {portfolioDate} · 每月更新</small></div>
      </section>
      ...
      <section className="holdings-preview shell">
        ...
        <aside className="allocation-card">
          <span>投資組合報酬</span>
          <div className="allocation-ring" aria-label={`累積報酬百分之 ${portfolioSnapshot.totalReturn.toFixed(1)}`}><b>{formatPercent(portfolioSnapshot.totalReturn, 1)}</b><small>累積報酬</small></div>
        ...
        <div className="shell">
          ...
          <div className="performance-grid">
            <div className="performance-bars" aria-label="XIRR 比較圖表">
              <div className="year-bar"><div className="bar-value" style={{ height: `${portfolioSnapshot.xirr * 8}px` }}><span>{formatPercent(portfolioSnapshot.xirr)}</span></div><small>投資組合</small></div>
            ...
            <div className="performance-copy"><strong>{formatPercent(portfolioSnapshot.xirr)}</strong><p>投資組合 XIRR；相同現金流期間的 {portfolioSnapshot.benchmark} 為 {formatPercent(portfolioSnapshot.benchmarkXirr)}。</p><small className="date-text">截至 {portfolioDate} 的已驗證快照 · 每月更新。</small><Link className="button button-white" href="/zh-tw/performance">查看績效 <span className="arrow-icon" aria-hidden="true">↗︎</span></Link></div>
          </div>
        </div>
      </section>
