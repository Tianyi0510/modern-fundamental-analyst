...
export function PortfolioPageContent({ locale }: { locale: Locale }) {
  ...
  return <main className="portfolio-page"><SiteHeader locale={locale} />
    ...
    <p className="data-note portfolio-source-note shell">{locale === "en" ? <>Source: <a href={portfolioSnapshot.sourceUrl} target="_blank" rel="noreferrer">“{portfolioSnapshot.source}”</a>; the verified snapshot is synchronized to this site monthly. Prices and market values use closing prices as of {asOf} and are not live quotes. Cash and external funding are excluded.</> : locale === "zh-tw" ? <>資料來源：<a href={portfolioSnapshot.sourceUrl} target="_blank" rel="noreferrer">「{portfolioSnapshot.source}」</a>，已驗證快照每月同步至本網站。價格與市場價值均採用 {asOf} 收盤價，並非即時報價；不包含現金與外部資金流。</> : <>数据来源：<a href={portfolioSnapshot.sourceUrl} target="_blank" rel="noreferrer">“{portfolioSnapshot.source}”</a>，已验证快照每月同步至本网站。价格与市场价值均采用 {asOf} 收盘价，并非实时报价；不包含现金与外部资金流。</>}</p><SiteFooter locale={locale} /></main>;
}
