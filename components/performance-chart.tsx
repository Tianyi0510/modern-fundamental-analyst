...
export function PerformanceChart({ locale }: { locale: Locale }) {
  ...
  return <figure className={styles.figure} aria-labelledby="performance-chart-title">
    <figcaption><h3 id="performance-chart-title">{text.title}</h3><p>{text.note}</p></figcaption>
    <ul className={styles.legend} aria-label={text.title}>
      <li><span className={styles.portfolioKey} />{text.portfolio}</li>
      <li><span className={styles.benchmarkKey} />SPY</li>
    </ul>
    <p>{text.axis}</p>
