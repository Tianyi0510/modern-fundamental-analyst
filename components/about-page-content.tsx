...
export function AboutPageContent({ locale }: { locale: Locale }) {
  ...
  return <main className="about-page" id="main-content"><SiteHeader locale={locale} />
    ...
    {text.sections.map((section) => <section className="about-section shell" key={section.label}><div className="about-section-heading"><span>{section.label}</span><h2>{section.title}</h2></div><div className="about-copy">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>)}
